...
MAMBA_NATIVE_AVAILABLE = MAMBA2_AVAILABLE or MAMBA1_AVAILABLE
print(f"Mamba2: {'available' if MAMBA2_AVAILABLE else 'not found'} | Mamba1: {'available' if MAMBA1_AVAILABLE else 'not found'}")
...
class SSMBlock(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, d_state: int = 128,
        ...
        if use_mamba1 and MAMBA1_AVAILABLE:
            self.ssm = Mamba(
                d_model=d_model,
                d_state=d_state,
                d_conv=4,
            ...
            self.ssm = wrap_mamba1_with_bounded_dt(self.ssm, dt_min=dt_min, dt_max=dt_max)
        else:
...
class TransformerSSMDenoise(nn.Module):
    ...
    def __init__(
        ...
        use_even_skip: bool = False,  # Skip connections over even layers (dual mode only)
        use_mamba1: bool = False  # Use Mamba v1 instead of Mamba2 for SSM layers
    ):
        super().__init__()
        ...
        self.use_even_skip = use_even_skip
        ...
        if self.baseline_type in ["ssm", "dual", "jamba"]:
            ...
            for i in range(n_layers):
                if self.baseline_type == "jamba":
                    ...
                    self.ssm_layers.append(SSMBlock(d_model, dropout, use_brake_mode=True, use_mamba1=use_mamba1))
                    ...
                    use_brake = False
                    self.ssm_layers.append(SSMBlock(ssm_dim, dropout, use_brake_mode=use_brake, use_mamba1=use_mamba1))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.n_layers):
            if self.baseline_type == "dual":
                # Skip connection over even layers
                if self.use_even_skip and i % 2 == 0:
                    x_skip = x
                ...
                # Add skip connection after even layers
                if self.use_even_skip and i % 2 == 0:
                    x = x + x_skip
